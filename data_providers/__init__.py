...
from typing import Any, Dict, Generator, Hashable, Iterable, Mapping, \
...
class BaseDataProvider(with_metaclass(ABCMeta)):
    """
    Implementation of the flywheel pattern.  Bulk-loads data from a
    backend, and parcels them out on an as-needed basis.

    See ``README.rst`` for more information on how to work with data
    providers.
    """
    def __init__(self):
        ...
        """
        Caches data loaded from the backend so that it can be accessed
        easily.
        """

        self._pending_load_keys = set() # type: Set[Hashable]
        """
        Stores lookup keys for values that haven't been loaded yet.

        Note: This value should be a set; if you find yourself wanting
        to use a dict here, :py:class:`BaseDataProviderDelegate` is
        probably a better fit for your use case.
        """

        self._pending_cache_keys = default_dict(set) # type: Dict[Hashable, Set[Hashable]]
        """
        Stores cache keys for values that haven't been loaded yet.

        This is used to gracefully handle cases where the backend
        doesn't return data for one or more load keys.
        """
    ...
    def __getitem__(self, value):
        ...
        :raise:
            - :py:class:`ValueError` if the value wasn't registered
              first.
        """
        # Determine the key we will use to look up the value in the
        # cache.
        ...
        try:
            ...
            if load_key is None:
                ...
                # undermine the whole point of using a flywheel in the
                # first place!
                if load_key not in self._pending_load_keys:
                    raise with_context(
                        exc =
                            ValueError(
                                'Attempting to get data for '
                                'unregistered value {value!r}.'.format(
                                    value = value,
                                ),
                            ),
    ...
    def fetch_from_backend(self, load_keys):
        # type: (Set[Hashable]) -> Union[Mapping[Hashable, Any], Generator[Tuple[Hashable, Any]]]
        """
        ...
        :return:
            ...
            The keys in the resulting mapping should correspond to the
            data provider's cache keys (from :py:meth:`gen_cache_key`).
        """
    ...
    def gen_empty_result(self):
        # type: () -> Any
        """
        Generates an empty result that is returned if the backend
        cannot (or will not) return data for a particular key.

        You may override this method in a subclass if desired.  This
        method may return any value, or even raise an exception,
    ...
    def register(self, values):
        # type: (Iterable) -> None
        """
        Registers a set of values so that the data provider can plan
        out the bulk queries it needs to execute against the backend.
        """
        for v in values:
    ...
    def warm_cache(self):
        """
        Warms the cache, fetching data for any keys that haven't been
        loaded yet.

        Note: the data provider will automatically load data from the
        backend as needed.  In general, you shouldn't have to invoke
        this method, though it can be useful in particular during
        debugging.
        """
    ...
    def _create_cache(self):
        ...
        restriction is that it must return some kind of mutable
        mapping.
        """
        return {}

    def _add_to_cache(self, cache_key, cache_value):
        # type: (Hashable, Any) -> None
        """
        Adds a value to the cache.

        If there is an existing value with the same key, it is
        overwritten.
    ...
    def _get_from_cache(self, cache_key):
        # type: (Hashable) -> Any
        """
        Attempts to retrieve the data from the cache for the specified
        key.
    ...
    def _add_to_key_registry(self, value):
        """
        ...
        if load_key is not None:
            ...
            # If we can't cache data for a value, then there would be
            # no point in loading data for it.
            # Also, we don't need to register a value if we already
            # have cached data for it.
            cache_key = self.gen_cache_key(value)
    ...
    def _get_keys_to_load(self, hint):
        # type: (Hashable) -> Set[Hashable]
        """
        Used by :py:meth:`_load_data` to decide which values to send to
        the backend.

        By default, this method returns all pending load keys, but you
        may override this method in a subclass if you want to use a
    ...
    def _load_batch(self, load_keys):
        # type: (Set[Hashable]) -> None
        """
        Bulk-loads data from the backend, only for the specified keys.
        """
        loaded = self.fetch_from_backend(load_keys)

        # :py:meth:`fetch_from_backend` is expected to return a
        # mapping, or at least a value that quacks like one.
        if isinstance(loaded, Generator):
            ...
            iterator = iteritems(loaded)

        for cache_key, cache_value in iterator:
            # The backend can overwrite any existing cached values.
            # You may override :py:meth:`_add_to_cache` in a subclass
            # if this is not desirable behavior.
            self._add_to_cache(cache_key, cache_value)

        # Fill in any missing cache keys that weren't included in the
        # result from the backend.
        for lk in load_keys:
            for ck in self._pending_cache_keys[lk]:
                if ck not in self._cache:
                    self._add_to_cache(ck, self.gen_empty_result())
            self._pending_cache_keys.pop(lk, None)

        self._pending_load_keys -= load_keys


class MutableDataProviderMixin(with_metaclass(ABCMeta, BaseDataProvider)):
    """
    A mixin for data provider classes that allow external code to
    modify the contents of the cache.

    Any values set this way will override values returned by the
    backend.
    """
    def __init__(self):
        super(MutableDataProviderMixin, self).__init__()

        self._override_cache_keys = set() # type: Set[Hashable]
    ...
    def __setitem__(self, value, cache_value):
        ...
        if load_key is not None:
            ...
            if cache_key is not None:
                ...
                # Since we are setting the cached value explicitly, we
                # do not need to backfill it when :py:meth:`_load_data`
                # runs.
                pending_cache_keys = self._pending_cache_keys[load_key]
                pending_cache_keys.discard(cache_key)

                # If there are no [other] registered cache keys
                # associated with this load key, then we won't have to
                # load any data for that key (i.e., we can drop it).
                if not pending_cache_keys:
                    self._pending_load_keys.discard(load_key)

                # Allow :py:meth:`__setitem__` to override values
                # more than once.
    ...
    def _replace_cached_value(self, cache_key, cache_value):
        # type: (Hashable, Any) -> None
        """
